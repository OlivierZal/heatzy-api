 ...
 * registry cycle failed: the session is established and the credentials
 * persisted, yet the registry could not be verified against the
 * server. Thrown by `authenticate()` with the cycle's own failure (a
 ...
 * transport failure during a sign-in over a PRE-EXISTING live token
