...
Nothing is dropped quietly, and nothing storms your logger: a listing drop and a skipped `/devdata` read are each reported as a **streak** — one `logger.error` line when it starts (the listing's names every dropped device and its verdict; the device read's carries the error, which names the failing paths and the values received), again when its reason changes and every sixty identical cycles, then one `logger.log` line when it recovers. A failed round-trip still reaches the `onRequestError` lifecycle event. To detect a partial cycle programmatically, compare what `fetch()` returned against the registry:
