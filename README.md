...
> [!IMPORTANT]
> The `SettingManager` receives the user token **and the account password** as plain strings. You are responsible for backing it with secure storage (encrypted settings store, OS keychain, …) — do not write it to a world-readable file. The SDK redacts credentials from everything it emits: its own log output, and the request/response snapshot carried by a thrown `HttpError` — the `X-Gizwits-User-token` header, the `/login` body's username and password, query parameters, and the response's headers and body alike — so an error object handed to a host logger, or pasted into a diagnostic report, carries no secret.
...
- A device whose `/devdata` read fails — a transient 5xx that outlived the retry rung, a payload whose required `mode` names a label this SDK cannot map — keeps the model it already had (stale data beats no data) and is skipped when it has never been seen. Its binding stays in the returned list, so the next cycle reads it again.
- A read value outside the vocabulary this SDK knows is NOT a failure: `/devdata` is checked for its wire types only, and the facades answer `null` for a `cur_mode` or a `derog_mode` they do not model. Every spelling Heatzy publishes for a mode it reports is translated to the one this SDK writes: the V1 Pilote's Chinese labels, and the `off` the vendor's 2020 document lists where its 2018 one spells `stop`. A switch is read in any of its three declared forms — `0`/`1`, `bool`, or the `off`/`on` labels of an enum-declared datapoint — and written as `0`/`1`. On a Pro, `currentSignal` reads the pilot-wire order the module is sending right now, which its own regulation can change under one commanded mode.

Nothing is dropped quietly, and nothing storms your logger: a listing drop and a skipped `/devdata` read are each reported as a **streak** — one `logger.error` line when it starts (the listing's names every dropped device and its verdict; the device read's carries the error, which names the failing paths and the values received), again when its reason changes (the failing paths for a refused payload, not the values received) and every sixty identical cycles, then one `logger.log` line when it recovers. While a device's streak is open, the per-attempt line the request pipeline writes for a failed HTTP call is held back for that device's read. A failed round-trip still reaches the `onRequestError` lifecycle event. To detect a partial cycle programmatically, compare what `fetch()` returned against the registry:
