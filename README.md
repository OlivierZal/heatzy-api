...
- A read value outside the vocabulary this SDK knows is NOT a failure: `/devdata` is checked for its wire types only, and the facades answer `null` for a `cur_mode` or a `derog_mode` they do not model. The V1 Pilote's Chinese mode labels are translated to their Latin mode.
