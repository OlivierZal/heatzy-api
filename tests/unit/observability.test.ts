...
describe('sensitive data redaction', () => {
  ...
  it('redacts auth headers in request data', () => {
    const config = createConfig({
      headers: {
        ...
        'X-Gizwits-User-token': 'abc123',
      },
    })
    const call = new APICallRequestData(config)
    const { headers } = parseLog(call.toString())

    expect(headers['X-Gizwits-User-token']).toBe('******')
  ...
  it('redacts sensitive keys inside form-encoded string bodies', () => {
    // Credentials posted as a form-encoded string
    // (URLSearchParams.toString()) bypass the object-key redaction.
    // Without explicit string handling in `redactValue()`, the entire
    // body — including `password=...` and `username=...` — would leak
    // verbatim into the request log lines.
    const config = createConfig({
